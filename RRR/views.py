    ...
    if (request.method == "POST"):
        ...
        if (str(x) == 'nan'):
            ...
            if (converted_dt + timedelta(hours=3) <= datetime.now()):
                ...
                user_data.loc[user_row[2], 'history'] = ','.join(
                    [str(info), str(x)])
                user_data.to_csv('user_data.csv', index=False)
        ...
        file.to_csv("ratings.csv", index=False)
        print("No of {} star rating is : {}".format(
            rate, file.loc[line-2, rate]))
    ...
    if (request.method == "POST"):
        if 'choice' in request.POST:
            ...
            for each_rest in reader:
                temp = []
                cuisine_list = set(
                    map(str.strip, each_rest['Cuisines'].split(',')))
...
def reccomendation(request):
    ...
    restaurant_file.close()
    return render(request, 'reccomendation.html', {'showModal': True, 'data': restaurant_names, 'cuisine_list': cuisine_list, 'user_name': user_row[0]})

    # data=res_suggestion(line-2)
    # data['data']=final

def suggestion2(request):
    global restaurant_names, line
    if (request.method == "POST"):
        if 'choice' in request.POST:
            # print("Choice")
            ...
            # print(sugg)
            sugg.sort(key=lambda x: x[2], reverse=True)
            return render(request, 'reccomendation.html', {'data': restaurant_names, 'sugg': sugg,'cuisine_list': cuisine_list})
        elif 'default' in request.POST:
            print("Default")
            sugg = res_suggestion(line-2)
            return render(request, 'reccomendation.html', {'data': restaurant_names, 'sugg': sugg,'cuisine_list': cuisine_list})   
